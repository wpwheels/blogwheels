...
import { gradientAttribute, gradientSlug } from "../../common/utils-gradient";
...
export default ({ attributes: { className }, setAttributes, clientId }) => {
	...
	const getGradientValue = () =>
		currentGradient ? getGradientValueBySlug(gradients, currentGradient) : null;

	// Returns a gradient slug by its value.
	const getGradientSlugByValue = (value) =>
		gradientSlug(gradientAttribute(value, gradients));

	// Define the gradient picker settings.
	const settings = {
		label: __("Gradient Outline", "blogwheels"),
